...
def performance_plot(ax, base_log_dir="logs"):
    lines = []
    for method, color in zip(["self_paced", "random", "default", "wasserstein", "gradient"],
                             ["C7", "C3", "C2", "C0", "C1"]):
    ...
    for method, color in zip(["goal_gan", "alp_gmm", "acl", "plr", "vds"], ["C8", "C9", "C4", "C5", "C6"]):
        exp = PointMass2DExperiment(base_log_dir, method, "ppo", {}, seed=0)
        log_dir = os.path.dirname(exp.get_log_dir())
        lines.append(add_plot(log_dir, ax, color))
    ...
    ax.set_xticks([0, 200000 / PointMass2DExperiment.STEPS_PER_ITER,
                   400000 / PointMass2DExperiment.STEPS_PER_ITER, 600000 / PointMass2DExperiment.STEPS_PER_ITER,
                   800000 / PointMass2DExperiment.STEPS_PER_ITER])
    ax.set_xticklabels(["$0$", "$200$K", "$400$K", "$600$K", "$800$K"])
    ax.set_xlim([0, 200])
...
def currot_plot(seed, iterations, axs=None, path=None, colors=None, s=10, base_log_dir="logs", alpha=0.2):
    ...
    for i, iteration in enumerate(iterations):
        ...
        ax.scatter(teacher.teacher.current_samples[:, 0], teacher.teacher.current_samples[:, 1], s=s, alpha=alpha,
                   color=colors[i])
...
def gradient_plot(seed, iterations, colormap, axs=None, path=None, s=10, base_log_dir="logs", alpha=0.2):
    exp = PointMass2DExperiment(base_log_dir, "gradient", "ppo", {}, seed=0)
    log_dir = os.path.dirname(exp.get_log_dir())
    seed_path = os.path.join(log_dir, "seed-%d" % seed)

    with open(os.path.join(seed_path, "context_trace.pkl"), "rb") as f:
        context_trace = pickle.load(f)
    ...
    for ax, interp_alpha, distribution in zip(axs, *context_trace):
        ax.scatter(distribution[:, 0], distribution[:, 1], s=s, alpha=alpha, color=colormap(interp_alpha))
...
def full_performance_plot(ax=None, path=None, base_log_dir="logs"):
    ...
    ax.tick_params(axis='both', which='minor', labelsize=TICK_SIZE, pad=1)
    ax.set_xlabel("Step", fontsize=FONT_SIZE, labelpad=2)
    ax.grid()
    ax.set_ylabel("Cum. Disc. Ret.", fontsize=FONT_SIZE, labelpad=2)

    if show:
        f.legend(lines,
                 [r"\sprl", "Random", "Default", r"\currot", r"\gradient", r"\goalgan", r"\alpgmm", r"\acl", r"\plr",
                  r"\vds"],
...
def precision_comparison(axs=None, path=None, base_log_dir="logs"):
    ...
    for i in range(0, 2):
        ...
        axs[i].set_xticks([0, 200000 / PointMass2DExperiment.STEPS_PER_ITER,
                           400000 / PointMass2DExperiment.STEPS_PER_ITER, 600000 / PointMass2DExperiment.STEPS_PER_ITER,
                           800000 / PointMass2DExperiment.STEPS_PER_ITER])
        axs[i].set_xticklabels(["$0$", "$200$K", "$400$K", "$600$K", "$800$K"])
        axs[i].set_xlim([0, 200])
        axs[i].set_xlabel("Step", fontsize=FONT_SIZE, labelpad=2)
        axs[i].set_ylabel("Gate Position" if i == 0 else "Gate Width", fontsize=FONT_SIZE, labelpad=1)
        axs[i].tick_params(axis='both', which='major', labelsize=TICK_SIZE, pad=1)
        axs[i].grid()
...
def combined_plot(path=None, base_log_dir="logs", single_column=False):
    f = plt.figure(figsize=(5.5, 1.4) if single_column else (4.3, 1.4))
    ax1 = f.add_axes([0.05 if single_column else 0.0625, 0.214,
                      0.27 if single_column else 0.26, 0.58 if single_column else 0.5])
    ax2 = f.add_axes([0.38 if single_column else 0.39, 0.214,
                      0.27 if single_column else 0.26, 0.58 if single_column else 0.5])
    ax3 = f.add_axes([0.715 if single_column else 0.722, 0.214,
                      0.27 if single_column else 0.26, 0.58 if single_column else 0.5])
    ...
    f.legend(lines,
             [r"\sprl", "Random", "Default", r"\currot", r"\gradient", r"\goalgan", r"\alpgmm", r"\acl", r"\plr",
              r"\vds"],
             fontsize=FONT_SIZE, loc='upper left', bbox_to_anchor=(-0.005, 1.03) if single_column else (0.16, 1.03),
             ncol=10 if single_column else 5, columnspacing=0.6, handlelength=0.9, handletextpad=0.25)
...
if __name__ == "__main__":
    ...
    combined_plot(base_log_dir=base_log_dir, single_column=single_column,
                  path=f"{dir_name}/point_mass_precision+performance.pdf")
