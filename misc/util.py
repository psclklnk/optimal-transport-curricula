...
def get_seed_performance(base_log_dir, max_seed=None):
    seed_performances = []
    ...
    seeds = sorted([int(d.split("-")[1]) for d in os.listdir(base_log_dir) if d.startswith("seed-")])
    ...
    if len(seed_performances) > 0:
        ...
        return np.array(iterations[0: min_length]), \
            np.array([seed_performance[0: min_length] for seed_performance in seed_performances])
    else:
        return [], []


def add_plot(base_log_dir, ax, color, max_seed=None, marker="o", markevery=3):
    perf_data = get_seed_performance(base_log_dir, max_seed=max_seed)

    if perf_data is not None:
        iterations, seed_performances = perf_data
        if len(seed_performances) > 0:
...
def select_best_hps(experiment, learner, teacher, params, log_dir):
    ...
    return [full_params[bi] for bi in best_idxs], [performances[bi] for bi in best_idxs]
...
if __name__ == "__main__":
    ...
    params, perfs = select_best_hps(SparseGoalReachingExperiment, "sac", "gradient",
                                    [{"GRADIENT_EPS": 0.05, "GRADIENT_DELTA": 0.8},
                                     {"GRADIENT_EPS": 0.05, "GRADIENT_DELTA": 0.6},
                                     {"GRADIENT_EPS": 0.05, "GRADIENT_DELTA": 0.4},
                                     {"GRADIENT_EPS": 0.1, "GRADIENT_DELTA": 0.8},
                                     {"GRADIENT_EPS": 0.1, "GRADIENT_DELTA": 0.6},
                                     {"GRADIENT_EPS": 0.1, "GRADIENT_DELTA": 0.4},
                                     {"GRADIENT_EPS": 0.2, "GRADIENT_DELTA": 0.8},
                                     {"GRADIENT_EPS": 0.2, "GRADIENT_DELTA": 0.6},
                                     {"GRADIENT_EPS": 0.2, "GRADIENT_DELTA": 0.4}], "logs")
