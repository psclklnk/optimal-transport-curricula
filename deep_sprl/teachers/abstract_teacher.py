    ...
    def __init__(self, env, teacher, discount_factor, context_visible, reward_from_info=False,
        ...
        if context_visible:
            ...
            if isinstance(self.env.observation_space, gym.spaces.Dict):
                ...
                #     if key == "observation":
                #         low_ext = np.concatenate((value.low, -np.inf * np.ones_like(context)))
                #         high_ext = np.concatenate((value.high, np.inf * np.ones_like(context)))
                #         new_spaces[key] = gym.spaces.Box(low=low_ext, high=high_ext)
                #     else:
    ...
    def step(self, action):
        ...
        if self.context_visible:
            ...
            if isinstance(obs, dict):
                ...
                #     if k == "observation":
                #         ext_obs[k] = np.concatenate((v, self.processed_context))
                #     else:
                #         ext_obs[k] = v
                # obs = ext_obs
            else:
