...
class NadarayaWatsonPy:
    ...
    def __init__(self, contexts, returns, lengthscale=None, custom_distance_function=None):
        ...
        self.custom_distance_function = custom_distance_function

    def squared_distance_function(self, x):
        if self.custom_distance_function is None:
            diffs = x[..., None, :] - self.contexts
            dists = np.sum(np.square(diffs), axis=-1)
            return dists, 2 * diffs
        else:
            res = self.custom_distance_function(x[..., None, :], self.contexts[None, ...])
    ...
    def predict_individual(self, x, with_gradient=False):
        ...
        squared_dists, squared_dist_grads = self.squared_distance_function(x)
        log_activations = -squared_dists / self.lengthscale
        ...
        if with_gradient:
            if squared_dist_grads is None:
                raise RuntimeError("Gradient computation not possible with user distance function that does not return "
                                   "gradient information")
            la_grads = -(1 / self.lengthscale) * squared_dist_grads
