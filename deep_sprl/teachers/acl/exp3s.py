...
class Exp3S:
    ...
    def __init__(self, n, eta, eps=0.2):
    ...
    def sample(self):
        ...
        # We re-normalize the probabilities since the choice function seems to be really picky when it comes to
        # normalization
        return np.random.choice(np.arange(self.weights.shape[0]), p=probs / np.sum(probs))
