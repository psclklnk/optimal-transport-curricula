        ...
        if count >= self.next_update:
            ...
            for _ in range(self.q_train_config["n_epochs"]):
                ...
                for i in range(self.q_train_config["batches_per_epoch"]):
                    ...
                    with torch.no_grad():
                        next_q_values = self.qs(next_obs, next_actions)
                    target_q_values = rewards + (1 - dones) * self.gamma * next_q_values
                    current_q_values = self.qs(obs, acts)
            ...
            disagreements = np.std(np.squeeze(self.qs(torch.from_numpy(states).type(torch.float32),
                                                      torch.from_numpy(actions).type(torch.float32)).detach().numpy()),
                                   axis=0)
...
class EnsembleQFunction(torch.nn.Module):
    ...
    def __init__(self, input_dim, layers, act_func, k, feature_extractor=None):
        super().__init__()
    ...
    def __call__(self, states: torch.Tensor, actions: torch.Tensor):
        if self.feature_extractor is not None:
            h = self.feature_extractor(states, actions)
        else:
