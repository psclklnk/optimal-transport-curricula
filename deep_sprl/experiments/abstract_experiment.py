...
class CurriculumType(Enum):
    ...
    def gradient(self):
        return self.value == CurriculumType.Gradient.value
    ...
    def from_string(string):
        ...
        elif string == str(CurriculumType.Gradient):
            return CurriculumType.Gradient
...
class DQNInterface(AgentInterface):
    ...
    def estimate_value_internal(self, inputs):
        with torch.no_grad():
            return torch.max(self.learner.policy.q_net.forward(torch.from_numpy(inputs)), axis=-1).values.numpy()

    def get_action(self, observations):
        flat_obs = np.reshape(observations, (-1, 1, observations.shape[-1]))
        flat_acts = self.learner.predict(flat_obs, deterministic=False)[0]
        return np.reshape(flat_acts, observations.shape[0:-1] + (-1,))

    def mean_policy_std(self, cb_args, cb_kwargs):
        return self.learner.exploration_rate
...
class Learner(Enum):
    ...
    def dqn(self):
        return self.value == Learner.DQN.value
    ...
    def create_learner(self, env, parameters):
        ...
        if self.ppo():
            ...
            model = DQN("MlpPolicy", env, **parameters["common"], **parameters[str(self)])
            interface = DQNInterface(model, obs_dim)
        else:
...
class AbstractExperiment(ABC):
    APPENDIX_KEYS = {"default": ["DISCOUNT_FACTOR", "STEPS_PER_ITER", "LAM"],
                     ...
                     CurriculumType.Wasserstein: ["DELTA", "METRIC_EPS", "ENT_LB"],
                     ...
                     CurriculumType.Gradient: ["GRADIENT_DELTA", "GRADIENT_EPS", "GRADIENT_ENT"]}
    ...
    def process_parameters(self):
        allowed_overrides = {"DISCOUNT_FACTOR": float, "MAX_KL": float, "STEPS_PER_ITER": int,
                             "LAM": float, "AG_P_RAND": float, "AG_FIT_RATE": int, "METRIC_EPS": float, "ENT_LB": float,
                             "GRADIENT_EPS": float, "GRADIENT_DELTA": float, "GRADIENT_ENT": float,
                             "AG_MAX_SIZE": self.parse_max_size, "GG_NOISE_LEVEL": float, "GG_FIT_RATE": int,
                             "GG_P_OLD": float, "DELTA": float, "EPS": float, "ACL_EPS": float,
